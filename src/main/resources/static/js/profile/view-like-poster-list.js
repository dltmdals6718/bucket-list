let likePosterPage = 1;
$(document).ready(function () {

    loadLikePosterOverview(likePosterPage);

    $("#like-poster-overview-prev-page-btn").click(function () {
        likePosterPage--;
        loadLikePosterOverview(likePosterPage);
    })

    $("#like-poster-overview-next-page-btn").click(function () {
        likePosterPage++;
        loadLikePosterOverview(likePosterPage);
    })
...
function loadLikePosterOverview(page) {

    $.ajax(`/api/like-posters?page=${page}`, {
        type: "GET",
        dataType: "json",
        success: function (data) {

            $("#like-poster-overview").empty();

            const posters = data.content;
            const pageInfo = data.page;

            likePosterOverviewPagination(pageInfo.number, pageInfo.totalPages);



            // 이전 페이지 버튼 상태 설정
            if (page <= 1) {
                $("#like-poster-overview-prev-page-btn").attr("disabled", true);
            } else {
                $("#like-poster-overview-prev-page-btn").removeAttr("disabled");
            }

            // 다음 페이지 버튼 설정
            if (page >= pageInfo.totalPages) {
                $("#like-poster-overview-next-page-btn").attr("disabled", true);
            } else {
                $("#like-poster-overview-next-page-btn").removeAttr("disabled");
            }
            ...
            for (const poster of posters) {
                ...
                $("#like-poster-overview").append(posterItem);
            }
...
function likePosterOverviewPagination(number, totalPage) {

    const paginationContainer = $("#like-poster-overview-pagination");
